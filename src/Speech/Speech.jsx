...
function Speech() {
  ...
  const start = () => {
    ...
    recognition.onresult = (e) => {
      ...
      if (finalTranscription.length > 1) {
        useRef.current =
          useRef.current + checkForText(" </br> " + finalTranscription);
        setTranscription(useRef.current);
        setInterim("");
        finalTranscription = "";
        interimTranscription = "";
      } else {
        setInterim(checkForText(interimTranscription));
        finalTranscription = "";
      }
    };
  ...
  const checkForText = (text = "") => {
    ...
    if (text.includes("Angel") || text.includes("angel")) {
      setAlert(true);
  ...
  return (
    <div>
      ...
      <div className="mb-3  grid grid-rows-1 grid-cols-3 ">
        ...
        <div className=" px-4">
          ...
          <button
            ...
            className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm h-6 px-4  py-0 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
          >
            {enableAutoScroll ? "AutoScrollEnabled" : "ScrollDisabled"}
          </button>
        </div>
      </div>
      ...
      <p
        id="message"
        rows="4"
        className=" overflow-scroll h-[50vh] block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        dangerouslySetInnerHTML={{ __html: transcript + interim }}
      ></p>
    </div>
  );
}
